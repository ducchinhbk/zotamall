...
function btnDNClick(link, data){
    if(isValidURL(link)){
        ...
        if(data == undefined || data.number_dn == undefined){
            currentLink = link;
            ajaxRequest('index.php?route=product/product_dn/getOrderedLink', 'GET', data,'json', function(data){
...
function sendDN(){
    ...
    ajaxRequest('index.php?route=product/product_dn/insertOrUpdate', 'POST', data,'json', function(data){
            alert(data);
        }, function(){}
    );
}
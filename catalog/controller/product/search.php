	...
	public function index() {
		...
		foreach ($categories as $category) {
			...
			$data['categories'][] = array(
                ...
                'product_count' => ($this->config->get('config_product_count') ? ' (' . $this->model_catalog_product->getTotalProducts($filter_cat_data) . ')' : '')
			);
		...
		if (!empty($setting['product'])) {
			$feature_products = array_slice($setting['product'], 0, 4); //just get 3 projects
			...
			foreach ($feature_products as $feature_product_id) {
				...
				if ($feature_product_info) {
					if ($feature_product_info['image']) {
						$image = $this->model_tool_image->resize($feature_product_info['image'], 192, 204);
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', 192, 204);
					}
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 292, 204);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 292, 204);
				}
